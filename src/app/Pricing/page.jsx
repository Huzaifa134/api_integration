...
const Price = () => {
  ...
  return (
    <div className="text-white">
      ...
      <div className="flex justify-center  items-center gap-x-10 text-sm border-2 rounded-[50px] w-[330px] h-15" style={{margin:"0 auto"}}>
        <button className="p-3 pl-7 pr-7 rounded-3xl" onClick={() =>handleButtonClick(1)} style={{backgroundColor:active==1?'#212124':'transparent'}}>
          Annaly <span style={{
            background: "linear-gradient(90deg,#ff27c3 14.12%,#2496ff 71.08%);-webkit-background-clip: text;-webkit-text-fill-color: transparent",
        }}>Save 20%</span>
        </button>
        <button className="p-3 pl-7 pr-7 rounded-3xl" onClick={() =>handleButtonClick(2)} style={{backgroundColor:active==2?'#212124':'transparent'}}>Monthly</button>
      </div>
      <PricingCard/>
    </div>
  );
